...
public class HiyoriController_js : MonoBehaviour, ISaveable
{
    ...
    public float ear_max_threshold = 0.15f;
    public float ear_min_threshold = 0.10f;
    ...
    void Start()
    {
        ...
        ear_left = 0.3913f;
        ear_right = 0.3773f;
        x_ratio_left = 0.5226f;
        y_ratio_left = 0.5200f;
        x_ratio_right = 0.5533f;
        y_ratio_right = 0.3770f;
        mar = 0.3083f;
        mouth_dist = 86.0523f;
   ...
   /* public void UpdateTheValue()
    {
        ...
        pitch = jshook.pitch;
        yaw = jshook.yaw;
        ear_left = jshook.ear_left;
        ear_right = jshook.ear_right;
        //stringText.text = roll.ToString() + " " + pitch.ToString() + " " + yaw.ToString();
        //print(string.Format("unity Roll: {0:F}; Pitch: {1:F}; Yaw: {2:F}", roll, pitch, yaw));
    }
   */
    ...
    void LateUpdate()
    {
        ...
        parameter.Value = -Mathf.Clamp(jshook.yaw, -30, 30);

        // pitch
        parameter = model.Parameters[1];
        parameter.Value = Mathf.Clamp(jshook.pitch, -30, 30);

        // roll
        parameter = model.Parameters[2];
        parameter.Value = -Mathf.Clamp(jshook.roll, -30, 30);
        ...
        parameter.Value = jshook.eye_L_value;
        parameter = model.Parameters[4];
        parameter.Value = jshook.eye_R_value;

        //IrisMovement();
        parameter = model.Parameters[8];
        parameter.Value = jshook.eyeball_x;
        parameter = model.Parameters[9];
        parameter.Value = jshook.eyeball_y;

        //MouthOpening();
        parameter = model.Parameters[19];
        parameter.Value = jshook.mar_clamped;
